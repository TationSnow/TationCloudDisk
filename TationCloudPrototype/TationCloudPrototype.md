...
## 移动端主入口
- 底部导航固定为：首页 / 文件 / 分享 / 我的，中央为上传主动作。
- 当前选定方案：`Variant C / Utility Tray`，已应用到移动主流程页面用于整套预览。
- 方案 C 已调整为直角外框，中央上传按钮仅保留图标，不再显示“上传”文字。
- 底栏改为左、右、下三边贴合屏幕，内容区与底栏之间通过页面结构分层，不再保留外侧留白。

## 移动端页面地图
- 一级与主流程页：Home、Files、Preview、Upload、Share、Membership、My、Trash、Notifications、Billing、Versions、Login。
...
- 我的域二级页：Security & Devices、Theme & Language、Notification Preferences。
- 回收站二级页：Trash Detail。
...
- 在我的页以账号中心方式查看个人信息、会员状态与高频入口，并进入安全设备、主题语言和通知偏好。
